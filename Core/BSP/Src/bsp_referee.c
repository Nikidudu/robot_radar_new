...
ref_msg_t ref_get_msg_from_buffer(ref_frame_header_t header, uint8_t *frame_buffer, uint16_t len) {
    ...
    case REF_ROBOT_HP_CMD_ID:
		if (header.data_length >= sizeof(ref_game_robot_HP_ally_t)) {
			memcpy(&buffer_msg.data.robot_hp, data_buffer, sizeof(ref_game_robot_HP_ally_t));
        }
        break;
    ...
    case REF_ROBOT_DATA_CMD_ID:
        if (header.data_length >= sizeof(ref_game_robot_data_t)) {
