...
#define REMOTE_SOF1 0xA9
#define REMOTE_SOF2 0x53
...
void remote_ISR() {
    ...
    /* 5. Mode switch -> right_switch, using hardware value directly */
    g_remote_cmd.right_switch = (int8_t)(mode_sw + 1);

    /* 6. Reconstruct left_switch from fn1 + right_switch */
    if (fn1 == 1) {
        g_remote_cmd.left_switch = ge_LSW_CONFIG;
    } else if (g_remote_cmd.right_switch == ge_RSW_ALL_ON) {  // 3 = all on position
        g_remote_cmd.left_switch = ge_LSW_UNSAFE;
    } else {
        g_remote_cmd.left_switch = ge_LSW_STANDBY;
    }
    ...
    /* 10. Keyboard */
    g_remote_cmd.keyboard_keys = (uint16_t)(buf[17] | (buf[18] << 8));

    g_remote_cmd.last_time = HAL_GetTick();

    /* 11. Notify control task */
