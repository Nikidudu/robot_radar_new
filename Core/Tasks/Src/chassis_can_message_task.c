...
void chassis_can_message_task(void *argument) {
    ...
    while(1) {
    	...
    	if (chassis_ctrl_data.forward == 0 && chassis_ctrl_data.horizontal == 0) {
//    		spin_limit += SPINSPIN_SPEED_BOOST;
    	}
...
#else
	if(supercap.supercap_enabled){
		*lvl_max_speed = MAX_SPEED_SUPERCAP;
		*lvl_max_accel = MAX_ACCEL_SUPERCAP;
		*lvl_max_spin  = CHASSIS_YAW_MAX_RPM_SUPERCAP;
	}else{
