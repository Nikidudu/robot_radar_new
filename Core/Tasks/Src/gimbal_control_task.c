...
void yaw_init() {
	...
	yaw_motor.rpm_pid.kff = YAWRPM_KFF;
...
//			imu_heading.pit, &prev_pit,0);
	...
	float temp_pit_output = pitch_motor->rpm_pid.output + PITCH_CONST;
	if (temp_pit_output < -20000.0f) temp_pit_output = -20000.0f;
	if (temp_pit_output >  20000.0f) temp_pit_output =  20000.0f;
...
void yaw_control(motor_data_t *yaw_motor) {
	...
	if (control_mode == SBC_CTRL_MODE) {
		float target_yaw = imu_heading.yaw + gimbal_ctrl_data.delta_yaw;
		while (target_yaw > PI)  target_yaw -= 2.0f * PI;
		while (target_yaw < -PI) target_yaw += 2.0f * PI;
		imu_angle_pid(target_yaw, imu_heading.yaw, yaw_motor, imu_heading.gyro_raw_yaw, 1);
	} else {
#endif
